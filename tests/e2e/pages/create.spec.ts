...
test.describe("the create page", () => {
    ...
    test("should allow selecting a closing date", async ({ page }) => {
        const createPage = new CreatePage(page);
        await createPage.selectDate("future");
        const now = new Date();
        const monthAfterNext = now.getMonth() + 2;
        const monthNames = [
            "January",
            "February",
            "March",
        ...
        await expect(createPage.locators.dateInput).toHaveValue(`01 ${monthNames[monthAfterNext]} 2025, 23:55 PM`);
    });
});
